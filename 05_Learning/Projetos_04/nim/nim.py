...
class NimAI():
    ...
    def update_q_value(self, state, action, old_q, reward, future_rewards):
        ...
        target_q_value = reward + future_rewards
        new_q_value = old_q + learning_rate * (target_q_value - old_q)
    ...
    def best_future_reward(self, state):
        ...
        `state`, return 0.
        """
        a_s = Nim.available_actions(state)
        # If there are no available actions
        if not a_s:
            return 0

        best_action = None
        best_action = max(self.q.get((tuple(state), a), 0) for a in a_s)
        return best_action
